...
# Cache: (satellite, model_type) -> (model, scaler, encoder)
_CACHE = {}
...
def _artifact_paths(satellite: str, model_type: str = "xgb"):
    sat = (satellite or "").upper().strip()
    mt  = (model_type or "xgb").lower().strip()
    model_file   = MODELS_DIR / f"{sat}_model_{mt}.joblib"
    scaler_file  = MODELS_DIR / f"{sat}_feature_scaler_{mt}.joblib"
    encoder_file = MODELS_DIR / f"{sat}_target_encoder_{mt}.joblib"
    return sat, mt, model_file, scaler_file, encoder_file

def _verify_artifacts_exist(model_file: Path, scaler_file: Path, encoder_file: Path):
    missing = [p.name for p in [model_file, scaler_file, encoder_file] if not p.exists()]
    if missing:
        raise FileNotFoundError(f"Missing artifact(s): {', '.join(missing)} in {MODELS_DIR}")

def load_artifacts(satellite: str, model_type: str = "xgb"):
    """
    Load model, scaler, and encoder for the selected satellite/model_type (cached).
    Filenames must follow the pattern: {SAT}_model_{type}.joblib, etc.
    """
    key = (satellite.upper(), model_type.lower())
    if key in _CACHE:
        return _CACHE[key]

    sat, mt, model_file, scaler_file, encoder_file = _artifact_paths(satellite, model_type)
    _verify_artifacts_exist(model_file, scaler_file, encoder_file)

    logger.info(f"Loading artifacts for satellite={sat}, model_type={mt}")
    model   = joblib.load(model_file)
    scaler  = joblib.load(scaler_file)
    encoder = joblib.load(encoder_file)

    _CACHE[key] = (model, scaler, encoder)
    return _CACHE[key]
...
def preprocess_single_row(row_dict, model, scaler):
    ...
    try:
        ...
        # --- engineered features (same as your original) ---
        if "koi_period" in df_row.columns and "koi_prad" in df_row.columns:
            p, r = df_row["koi_period"].iloc[0], df_row["koi_prad"].iloc[0]
            if pd.notna(p) and pd.notna(r) and r != 0:
                df_row["period_to_radius_ratio"] = p / (r**3)
        ...
        # Feature ordering to match training
        feature_names = _feature_order(model, scaler, df_row)
        vals = []
        for f in feature_names:
            if f in df_row.columns:
...
def predict_from_csv(csv_file_path, satellite: str, row_numbers=None, model_type: str = "xgb"):
    """
    Predict exoplanet classification for specific rows (or all rows if None)
    using artifacts selected by satellite (and model_type).
    Returns a DataFrame with predictions.
    """
    model, scaler, encoder = load_artifacts(satellite, model_type)
